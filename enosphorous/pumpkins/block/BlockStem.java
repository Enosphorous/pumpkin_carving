...
import enosphorous.pumpkins.block.BlockMelon.NaturalTypes;
import enosphorous.pumpkins.item.Items;

public class BlockStem extends BlockFlower
{
    /** Defines if it is a Melon or a Pumpkin that the stem is producing. */
    private final ItemStack fruitType;
    @SideOnly(Side.CLIENT)
    private Icon theIcon;

    protected BlockStem(int id, ItemStack fruitType)
    {
        super(id);
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (par1World.getBlockLightValue(par2, par3 + 1, par4) >= 9)
        {
            ...
            if (par5Random.nextInt((int)(25.0F / f) + 1) == 0)
            {
                ...
                {
                    if (par1World.getBlockId(par2 - 1, par3, par4) == this.fruitType.itemID)
                    {
                        return;
                    }
                    ...
                    if (par1World.getBlockId(par2, par3, par4 - 1) == this.fruitType.itemID)
                    {
                        return;
                    }
                    ...
                    {
                        par1World.setBlock(j1, par3, k1, this.fruitType.itemID, fruitType.getItemDamage() == 0 ? NaturalTypes.melon.ordinal() : NaturalTypes.getRandomPumpkinType(par5Random).ordinal(), 3);
                    }
                }
            }
        }
    ...
    public int getState(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        return l < 7 ? -1 : (par1IBlockAccess.getBlockId(par2 - 1, par3, par4) == this.fruitType.itemID ? 0 : (par1IBlockAccess.getBlockId(par2 + 1, par3, par4) == this.fruitType.itemID ? 1 : (par1IBlockAccess.getBlockId(par2, par3, par4 - 1) == this.fruitType.itemID ? 2 : (par1IBlockAccess.getBlockId(par2, par3, par4 + 1) == this.fruitType.itemID ? 3 : -1))));
    }
    ...
    public ArrayList<ItemStack> getBlockDropped(World world, int x, int y, int z, int metadata, int fortune)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                ret.add(new ItemStack(idPicked(world, x, y, z), 1, 0));
            }
        }
    ...
    public int idPicked(World par1World, int par2, int par3, int par4){
        return fruitType.getItemDamage() == 0 ? Items.melonSeeds.itemID : Items.pumpkinSeeds.itemID;
    }
